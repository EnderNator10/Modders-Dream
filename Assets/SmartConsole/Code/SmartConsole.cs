...
public class SmartConsole : MonoBehaviour
{
	...
	void Update()
	{
		   ...
		   && ( s_textInput.GetComponent<GUIText>() != null ) )
		{
			...
			s_textInput.GetComponent<GUIText> ().color = new Color (0, 100, 200);
		}
	...
	public static  void LuaCmd(string filename){
		NLua.Lua lua = new NLua.Lua ();
		lua.LoadCLRPackage ();
	}
	private static void InitialiseCommands()
	{
		RegisterCommand ("Lua.LoadFile", "Loads a lua file", LuaCmd);
	...
	private static void SetStringsOnHistoryElements()
	{
		for( int i = 0; i < k_historyLines; ++i )
		{
			...
			if( historyIndex >= 0 )
			{
				...
				s_historyDisplay[ i ].GetComponent<GUIText> ().color = new Color (0, 100, 200);
				i++;
			}
			else
			{
	...
	private static void LogHandler( string message, string stack, LogType type )
	{
		...
		string exceptPrefix 	= "[Exception]: ";
		string warningPrefix 	= "[Warning]:   ";
		string otherPrefix 		= "[Log]:          ";
